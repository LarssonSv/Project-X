...
public class CharacterControllerv2 : MonoBehaviour {
    ...
    private Vector3 CalculateVelocityChange(Vector3 inputVector) {
        ...
        if (inputVector.z > 0) {
            relativeVelocity.z *= (canRun && Input.GetKey(KeyCode.W)) ? runSpeed : walkSpeed;
        }
        else {
            relativeVelocity.z *= (canRun && Input.GetKey(KeyCode.W)) ? runBackwardSpeed : walkBackwardSpeed;
        }
        relativeVelocity.x *= (canRunSidestep && Input.GetKey(KeyCode.W)) ? runSidestepSpeed : sidestepSpeed;
